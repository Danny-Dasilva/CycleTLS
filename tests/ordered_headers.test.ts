...
test('Should correctly set header order', async () => {
    ...
    const defaultHeaders = await cycleTLS('https://pgl.yoyo.org/http/browser-headers.php', {
        ja3: ja3,
        userAgent: userAgent,
        headers: { "host": "pgl.yoyo.org","connection": "keep-alive", "cache-control": "no-cache"},
        proxy: ''
    });
    ...
    const customHeaderOrder = ["User-Agent", "Cache-Control", "Connection", "Host"]
    const customHeaders = await cycleTLS('https://pgl.yoyo.org/http/browser-headers.php', {
        ja3: ja3,
        userAgent: userAgent,
        headers: { "host": "pgl.yoyo.org","connection": "keep-alive", "cache-control": "no-cache"},
    ...
    const expectedresponseOrder = ["User-Agent", "Host", "Connection", "Cache-Control"]
