...
func TestTimeoutSuccess(t *testing.T) {
	...
	// Bump per-request Timeout to comfortably exceed httpbin's flake delays plus our retry backoff.
	resp := doHTTPBinRequestWithRetry(t, client, "http://httpbin.org/delay/1", cycletls.Options{
		Body:      "",
